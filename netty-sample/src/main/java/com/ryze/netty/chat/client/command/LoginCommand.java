...
public class LoginCommand extends AbstractCommand {
    ...
    public ChatInfo.Chat parseInput(String input) {
        ...
        ChatInfo.User user = ChatInfo.User.newBuilder().setUsername(username).setPassword(password).build();
        ChatInfo.LoginRequest loginRequest = ChatInfo.LoginRequest.newBuilder().setLoginUser(user).build();
        ChatInfo.Chat chat = ChatInfo.Chat.newBuilder().setMsgType(ChatInfo.Chat.MessageType.LOGIN_REQUEST).setLoginRequest(loginRequest).build();
        return chat;
